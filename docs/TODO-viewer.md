...
5. Sprawdzić czy `setup.sh` po sklonowaniu repo automatycznie konfiguruje domeny `.local` (lub `.test`) — czy viewer jest od razu dostępny pod `app.local/logs`
6. Dodać w README opis jak dodać kolejny serwis na nowym porcie (np. `localhost:3467`) — krok po kroku: docker-compose, proxy, /etc/hosts
